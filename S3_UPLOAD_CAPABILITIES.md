...
- **Fields**: each has a unique `fieldId`, `fieldName`, `required` flag, and a `fieldType` (`TEXT`, `TAG_GROUP`, `DATE`, `PERSONA`, `IGNORE`, defaults to `TEXT`); `TAG_GROUP` fields carry a `tagGroupTitle` matching an existing tag group name
